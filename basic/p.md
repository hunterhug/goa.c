...
如果一个问题，可以用一个算法在确定多项式时间内解决，它称为 P 问题(P：polynominal，多项式)。比如求1加到100的总和，它的时间复杂度是 O(n)，是多项式时间。

然而有些问题，只能用枚举的方式求解，时间复杂度是指数级别，非确定多项式时间，但是只要有一个解，我们能在确定多项式时间验证这个解是对的，这类问题称为 NP 问题。
...
多项式级别：类似于O(1)，O(log2n)，O(na)，规模 n 出现在底数的位置，我们称为多项式级的复杂度。

非多项式级别：类似于O(n!)，类似阶乘，指数乘方，规模 n 出现在顶部的位置，我们称为非多项式级的复杂度。
...
最后，我们一旦发现一个问题是 NPC 问题，那么我们很难去准确求出其解，只能暴力枚举，靠猜。