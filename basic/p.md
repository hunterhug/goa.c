...
类似于O(1)，O(log2n)，O(na)等复杂度，规模 n 出现在底数的位置，计算机能在确定的多项式时间解决，我们称为多项式级的复杂。

类似于O(n!)，指数乘方等复杂度，规模 n 出现在顶部的位置，计算机能在非确定的多项式时间解决，我们称为非多项式级的复杂度。（为什么叫非确定的多项式，比如指数级别的复杂度，随着 n 的变大，时间的增长是爆炸式非确定的。）

如果一个问题，可以用一个算法在确定的多项式时间内解决，它称为 P 问题(P：polynominal，多项式)。比如求1加到100的总和，它的时间复杂度是 O(n)，是确定的多项式时间。

然而有些问题，只能用枚举的方式求解，时间复杂度是指数级别，非确定的多项式时间，但是只要有一个解，我们能在确定的多项式时间验证这个解是对的，这类问题称为 NP 问题。
