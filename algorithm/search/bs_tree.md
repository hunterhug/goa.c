...
由于这种通用形式的二叉查找树实现甚少使用，大部分程序都使用了AVL树或红黑树，以上优化的建议就不提供代码实现了，理解即可。
...
二叉查找树的关键在于二分查找，其各种操作都依赖于查找，查找次数与树的高度有关，因此时间复杂度范围为 `log(n)~n`。

为了提高二叉查找树查找的速度，树的高度要尽可能的小。AVL树，红黑树正是相对平衡的二叉查找树，因为维持了树的平衡，树的高度大大减小，其查找，添加，删除元素平均时间复杂度提升到 `log(n)`。
