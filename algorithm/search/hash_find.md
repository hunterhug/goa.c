...
我们使用了 `current` 和 `old` 字段来表示伸缩前后的哈希表数组。因为在伸缩过程中，需要对哈希表中的元素重新哈希，假设元素数量特别多，那么速度是极慢的，不能忍受。我们使用这两个字段来进行延后处理，重新哈希的这个过程将是渐进的。

当 `reHash` 字段是 `true` 时表示正在重新哈希中，那么获取键值对时会先去老数组获取，老数组存在时将其移到新数组。如果键值对不存在，那么直接添加到新数组中。

我们还使用了 `lock` 来实现并发安全，防止并发增删元素或者数组伸缩时产生混乱。
