...
拿到哈希值之后，我们要对结果取余，方便定位到数组下标 `index`。如果数组的长度为 `len`，那么 `index = xxhash(key) mod len`。
...
因为恒等式 hash % 2^k = hash & (2^k-1)，表示截断二进制的位数，保留后面的 k 位：

所以  165 % 8 = 1010 0101 & 0000 0111 = 0000 0101 = 5
```

选择 `2^x` 长度虽然计算会更快，但是因为只保留二进制后面的 `k` 位，如果存在很多哈希值的值很大，位数超过了 `k` 位，而二进制后 `k` 位都相同，那么会导致大片哈希冲突。

即使如此，大概率下哈希值的二进制都不会超过 `k` 位，截断后 `k` 位并不会损失什么，因此编程语言 `Golang` 和 `Java` 最新实现都使用了这种 `2^x` 长度作为哈希表的数组长度。  

当然，我们最好选择素数(prime number)来做取余操作。素数也叫质数，除了 `1` 和自身，和其他所有数的
