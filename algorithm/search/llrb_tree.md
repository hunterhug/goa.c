...
要在子树 `b` 的的左子树中删除元素，这时树 `b` 根节点是红节点，其儿子 `a，c` 节点都为黑色节点，且两个黑色节点都是2节点，都没有左红孩子，那么直接变色即可（相当于 `2-3` 树：把父亲的一个值拉下来合并），如图：

![](../../picture/llrb_tree_delete1.jpg)

如果存在右儿子是3节点，有左红孩子，那么需要先变色后，对兄弟节点右旋，再对父亲节点左旋，最后变色（相当于 `2-3` 树：向3节点兄弟借值，然后重新分布），如图：

![](../../picture/llrb_tree_delete2.jpg)

红色左移可以总结为下图（被删除的节点在左子树，且进入的树根一定为红节点）：
...
为什么要红色左移，是要保证调整后，子树根节点 `h` 的左儿子或左儿子的左儿子有一个是红色节点，这样从 `h` 的左子树删除元素，才可以继续递归下去，递归的结局就是被删除的叶子节点变成一个红色节点，然后就可以直接删除了。

红色右移的步骤类似，如图（被删除的节点在右子树，且进入的树根一定为红节点）：
...
为什么要红色右移，同样是为了保证树根节点 `h` 的右儿子或右儿子的右儿子有一个是红色节点，往右子树删除元素可以继续递归下去。
...
下面是左倾红黑树从h的左子树或右子树删除元素的一次递归实例图：

![](../../picture/llrb_tree_delete_total.jpg)

可以对照着，继续阅读下面的左倾红黑树删除元素代码：
