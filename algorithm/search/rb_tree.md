...
因为 `2-3` 树的第二个特征，它是一颗完美平衡的树，所以树的高度非常的小。
...
我们的代码实现中，左倾红黑树的插入，需要逐层判断是否需要旋转和变色，复杂度为 `log(n)`，可能旋转和染色多次（当旋转变色后导致上层存在连续的红左链接），如图在箭头处添加新节点，出现了右红链接，然后要一直旋转变色到根节点（穿投到根节点的情况极少发生）：

![](../../picture/llrb_tree_example.jpg)

我们可以优化代码，使得在某一层旋转变色后，如果其父层没有连续的左红链接或者不需要变色，那么可以直接退出，不需要逐层判断是否需要旋转和变色。

对于 `AVL` 树来说，插入最多旋转两次，但其需要逐层更新树高度，复杂度为 `log(n)`。

按照插入效率来说，很多教程都说左倾红黑树会比 `AVL` 好一点，因为其不要求严格的平衡，会插入得更快点，但根据我们实际上的递归代码，两者都需要逐层向上判断是否需要调整，只不过 `AVL` 树多了更新树高度的操作，此操作影响了一点点效率，但我还是觉得两种树的插入效率都差不多。

在此，我们不再纠结两种平衡树哪种更好，在平均情况下，其实都差不多。
