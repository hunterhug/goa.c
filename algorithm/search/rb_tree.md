...
嗨，大家好，这篇红黑树教程是2020年初新冠疫情刚开始的时候写的，突觉很多教程都没有讲清红黑树到底是怎么实现的，甚觉可惜，于是放入[数据结构](https://hunterhug.github.io/goa.c)教程系列中。

在安静的夜晚中，喝杯茶，然后一点点手绘了红黑树相关的图例，辅以说明，以此篇作为数据结构教程系列的结尾之作，望大家不忘初心，牢记使命，静心，耐心，安心好好学习美妙的知识。 -- 2024年6月备注。

## 正文

某些教程不区分普通红黑树和左倾红黑树的区别，直接将左倾红黑树拿来教学，并且称其为红黑树，因为左倾红黑树与普通的红黑树相比，实现起来较为简单，容易教学。在这里，我们区分开左倾红黑树和普通红黑树。

红黑树是一种平衡的二叉查找树，它是一颗二叉树，同时是一颗平衡的二叉树。

红黑树为什么叫红黑树，因为其从 `2-3` 树或 `2-3-4` 树衍生而来，通过对二叉树节点进行染色，染色为红或黑节点，来模仿 `2-3` 树或 `2-3-4` 树的3节点和4节点，从而让树的高度减小。

数据结构中的 `2-3-4` 树对照实现的红黑树是普通的红黑树，而 `2-3` 树对照实现的红黑树是一种变种，称为左倾红黑树，其更容易实现（请看[2-3树和左倾红黑树](https://hunterhug.github.io/goa.c/#/algorithm/search/llrb_tree)的介绍）。

使用平衡树数据结构，可以提高查找元素的速度，我们在本章介绍 `2-3-4` 树，再用二叉树形式来实现 `2-3-4` 树，也就是普通的红黑树。

目前已经有更加完备的红黑树代码实现，请参考：[https://github.com/hunterhug/gomap](https://github.com/hunterhug/gomap/blob/master/README_ZH.md) 。
