...
选择排序，一般是指简单选择排序，它不像冒泡排序一样相邻地交换元素，每次只需交换一次，但是效率还是和冒泡排序一样的糟糕。

我们打扑克牌的时候，都会习惯性地从左到右扫描，然后将最小的牌放在最左边，然后继续从左到右扫描第二小的排，放在最小的牌右边，以此反复。
...
比较的次数和冒泡排序一样，因为扫描过程也是比较的过程，只不过交换的次数减少为每轮 1 次。时间复杂度仍然是：`O(n)`。

选择排序是一个不稳定的排序算法，比如数组：`[5 6 5 1]`，第一轮迭代时数字 `1` 就和数字 `5` 交换了位置，导致两个相同的数字排序后位置变了。
